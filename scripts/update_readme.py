...
def build_readme(ascii_table: str, contrib_grid: str, ascii_plot: str, ascii_hist: str) -> str:
    """
    Build README and inline pulsar SVG when present.

    Looks for scripts/pulsar.svg first (preferred), then pulsar.svg. If found,
    the SVG markup is inlined (XML prolog removed if present) so README always contains
    the fresh image that was generated by the Action at runtime.
    """
    svg_inline = ""
    candidates = ["scripts/pulsar.svg", "pulsar.svg"]
    for cand in candidates:
        if os.path.exists(cand):
            try:
                with open(cand, "r", encoding="utf-8") as fh:
                    raw = fh.read()
                # strip initial XML prolog if present (keeps inline safe)
                raw = re.sub(r'^\s*<\?xml[^>]+\?>\s*', '', raw, count=1)
                svg_inline = raw.strip()
                break
            except Exception:
                svg_inline = ""
    if svg_inline:
        ...
        # fallback to referencing script path (if inline not available)
        svg_block = (
            '<p align="center">\n'
    ...
    return (
        # SVG block is intentionally _outside_ the <pre> so it renders as an image on GitHub.
        svg_block +
        "<pre>\n"
...
def create_pulsar_svg(repo_weekly: Dict[str, List[int]],
                      repo_order: List[str],
    ...
    # Build array: rows x weeks
    mat = []
    for r in repo_order:
        w = repo_weekly.get(r, [0]*weeks)
        if len(w) < weeks:
            w = [0]*(weeks - len(w)) + w

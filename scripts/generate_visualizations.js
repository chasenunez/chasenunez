...
// Uses dynamic import for d3 (ESM) while script stays CommonJS.
...
// main runner as async IIFE so we can use dynamic import()
(async () => {
  let d3;
  try {
    // dynamic import of ESM d3
    const mod = await import('d3');
    // mod contains named exports; to emulate `const d3 = require('d3')`,
    // we keep the module namespace object directly.
    d3 = mod;
  } catch (err) {
    console.error('Failed to import d3 module dynamically:', err);
    process.exit(1);
  }

  // Build figure 1 data (traffic)
  const { traffic: fig1Traffic } = buildFigure1Data(cache, process.env.USERNAME || 'chasenunez', 365, 10);
  ...
  function makeFigure1(traffic, outPath) {
    ...
    const overlap = 8;
    const width = 960;
    const height = Math.max(200, groups.length * 17);
